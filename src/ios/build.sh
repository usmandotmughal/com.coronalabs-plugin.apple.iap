...
# Clean
xcodebuild -project "$path/IAPPlugin.xcodeproj" -configuration $CONFIG clean
checkError

# iOS
xcodebuild -project "$path/IAPPlugin.xcodeproj" -configuration $CONFIG -sdk iphoneos
checkError

# iOS-sim
xcodebuild -project "$path/IAPPlugin.xcodeproj" -configuration $CONFIG -sdk iphonesimulator
checkError
...
rm -rf "$OUTPUT_DIR/BuiltCryptohelperPlugin"

TARGET_NAME=plugin_iap_cryptohelper

lipo -create "$path"/build/$CONFIG-iphoneos/lib$TARGET_NAME.$OUTPUT_SUFFIX "$path"/build/$CONFIG-iphonesimulator/lib$TARGET_NAME.$OUTPUT_SUFFIX -output "$OUTPUT_DIR"/lib$TARGET_NAME.$OUTPUT_SUFFIX


build_plugin_structure "$OUTPUT_DIR/BuiltCryptohelperPlugin/iphone" iphoneos  " -extract armv7 -extract  arm64 "
build_plugin_structure "$OUTPUT_DIR/BuiltCryptohelperPlugin/iphone-sim" iphonesimulator  " -extract i386  -extract  x86_64 "

cp "$path/metadata_cryptohelper.lua" "$OUTPUT_DIR/BuiltCryptohelperPlugin/iphone/metadata.lua"
cp "$path/metadata_cryptohelper.lua" "$OUTPUT_DIR/BuiltCryptohelperPlugin/iphone-sim/metadata.lua"
